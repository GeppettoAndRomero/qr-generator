...
test.describe('covenants', () => {
  ...
  test('links to related tools and its own engineering-notes article (#177)', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(href, `related card ${i} is not a self-link`).not.toContain('/qr-generator/');
    ...
    if (await blogLink.count()) {
      await expect(blogLink).toHaveAttribute('href', /^https:\/\/runlocally\.app\/blog\/qr-generator\/$/);
    }
  });
  ...
  test('keeps no input text in the page URL after generating (#7)', async ({ page }) => {
    await page.goto('/qr-generator/');
  ...
  test('generates offline after the first online visit (#2)', async ({ page }) => {
    ...
    await page.reload();
    await waitReady(page);
    // Generate online so the qrcode chunk is fetched and cached too.
    await generateQr(page);
    ...
    try {
      ...
      await waitReady(page);
      await generateQr(page); // generate with no network at all
    } finally {
